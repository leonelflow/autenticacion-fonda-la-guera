...
async function sendAccessCodeEmail(to, code) {
  ...
  if (!transporter) {
    console.log("CÓDIGO DE ACCESO (modo dev):", code);
    return;
  }
...
function cleanExpiredTwoFactorChallenges() {
  ...
  for (const [challengeId, data] of pendingTwoFactor.entries()) {
    if (data.expiresAt < now) pendingTwoFactor.delete(challengeId);
  }
}
...
app.post("/api/sessionLogin", loginLimiter, async (req, res) => {
  try {
    ...
    if (!idToken) return res.status(400).json({ error: "Falta el ID token." });

    const decodedIdToken = await admin.auth().verifyIdToken(idToken);
    const authTime = decodedIdToken.auth_time * 1000;
    if (Date.now() - authTime > 5 * 60 * 1000) {
      return res.status(401).json({ error: "Inicio de sesión antiguo. Vuelve a iniciar sesión." });
    }
    if (!decodedIdToken.email) {
      return res.status(400).json({ error: "La cuenta no tiene correo electrónico." });
    }
    ...
    res.json({ status: "code_sent", email: maskEmail(decodedIdToken.email) });
  } catch (error) {
    console.error("sessionLogin 2FA error:", error);
    res.status(401).json({ error: "No se pudo iniciar la verificación en dos pasos." });
  }
});
...
app.post("/api/verifyAccessCode", loginLimiter, async (req, res) => {
  try {
    ...
    if (!challengeId || !pendingTwoFactor.has(challengeId)) {
      return res.status(401).json({ error: "No hay una verificación pendiente. Inicia sesión otra vez." });
    }
    if (!/^[0-9]{6}$/.test(code)) {
    ...
    if (Date.now() > challenge.expiresAt) {
      ...
      res.clearCookie("mfa_challenge", { httpOnly: true, secure: isProduction, sameSite: "strict", path: "/" });
      return res.status(401).json({ error: "El código venció. Inicia sesión otra vez." });
    }

    if (challenge.attempts >= 5) {
      pendingTwoFactor.delete(challengeId);
      res.clearCookie("mfa_challenge", { httpOnly: true, secure: isProduction, sameSite: "strict", path: "/" });
      return res.status(429).json({ error: "Demasiados intentos. Inicia sesión otra vez." });
    }
    ...
    const sessionCookie = await admin.auth().createSessionCookie(challenge.idToken, { expiresIn });
...
app.post("/api/orders", requireAuth, async (req, res) => {
  try {
    ...
    if (!customerName || !phone || !Array.isArray(items) || items.length === 0) {
      return res.status(400).json({ error: "Completa nombre, teléfono y productos." });
    }
    ...
    const calculatedTotal = cleanItems.reduce((sum, item) => sum + item.price * item.quantity, 0);
    ...
    res.status(201).json({ id: docRef.id, status: "pendiente", total: calculatedTotal });
  } catch (error) {
